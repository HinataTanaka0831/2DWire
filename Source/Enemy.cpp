...
Enemy::Enemy(VECTOR initPos, std::string filename, int t, int s)
	...
	,mvDirection(VGet(-1.0f, 0.0f, 0.0f))   // Å‰‚ÍXŽ²‚Ì‰E•ûŒü‚Ö“®‚­‚æ‚¤‚É‚µ‚Ä‚¨‚­
...
void Enemy::Move()
{
	...
	{
		// mvDirection.x ‚Ì•ûŒü‚Öi‚Þ‚æ‚¤‚É‚·‚é
		mvPosition.x += (float)MOVE_SPEED * mvDirection.x;
	}

	// ƒeƒNƒXƒ`ƒƒ‚ÉÀ•W‚ð“`‚¦‚é
...
void Enemy::Calcdamage()
{
	...
	if (pPlayer != nullptr)     // dynamic_cast ‚ð‚µ‚½Œã‚Í•K‚¸nullƒ`ƒFƒbƒN‚ðs‚¤
	{
		// “–‚½‚è”»’è
